// src/pages/SecurityComplianceAdvisor.jsx
import React, { useState, useRef, useEffect } from 'react';
import { motion } from 'framer-motion';

const SecurityComplianceAdvisor = ({ isDarkMode }) => {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      const promptContext = `The user is asking a question about AWS security or compliance: "${newUserMessage.text}".
      As an expert AWS Security and Compliance Advisor, provide a concise and accurate answer.
      If it's about best practices, list key actionable steps. If it's about compliance, explain the standard's relevance to AWS.
      Focus on practical, actionable advice.`;

      const response = await fetch('https://backend-project-190.onrender.com/api/ai/security-compliance', {
        method: 'POST',
      ...
      setMessages(prevMessages => [...prevMessages, { sender: 'ai', text: data.securityComplianceResponse || "Sorry, I couldn't provide security or compliance advice for that." }]);

    } catch (error) {
      console.error("Error getting AI security/compliance response:", error);
  ...
  return (
    ...
    >
      <h1 className={`text-3xl sm:text-4xl font-bold mb-8 ${headerColorClass}`}>
        AI Operations: Security & Compliance Advisor
      </h1>
      ...
      <div className={`flex-grow flex flex-col ${chatContainerBgClass} rounded-lg shadow-lg border border-gray-700 overflow-hidden`}>
        <div className="p-4 border-b border-gray-700 text-lg font-semibold text-center text-blue-300">
          Ask your security or compliance questions, and I'll provide expert advice.
        </div>
        <div className="flex-grow p-4 overflow-y-auto space-y-4">
          {messages.length === 0 && (
            ...
            >
              Hello! I'm your AI Security & Compliance Advisor. How can I assist you with securing your AWS resources or understanding compliance standards?
              <p className="text-xs mt-1 opacity-80">
                (e.g., "Best practices for IAM roles?", "Explain HIPAA compliance for AWS data," "How to secure S3 bucket access?")
              </p>
            </motion.div>
          )}
        ...
        <form onSubmit={handleSendMessage} className="p-4 border-t border-gray-700 flex">
          <input
            ...
            placeholder="e.g., 'How to secure data in S3?'"
...
export default SecurityComplianceAdvisor;

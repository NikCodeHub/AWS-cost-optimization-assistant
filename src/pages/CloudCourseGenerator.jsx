// src/pages/CloudCourseGenerator.jsx
import React, { useState, useRef, useEffect } from 'react';
import { motion } from 'framer-motion';

const CloudCourseGenerator = ({ isDarkMode }) => {
  ...
  const handleGenerateCourse = async (e) => {
    ...
    const newUserMessage = { sender: 'user', text: userGoal.trim() };
    setMessages(prevMessages => [...prevMessages, newUserMessage]);
    setUserGoal('');
    setLoading(true);

    try {
      const promptContext = `As an expert AI Cloud Curriculum Designer, your task is to generate a step-by-step learning path for the user based on their learning goal. The course should be practical, actionable, and cover the essentials.
      
      Learning Goal: "${newUserMessage.text}"
      ...
      3.  **Module 2: [Topic Name]**
          * ... (repeat structure for 3-5 modules total, progressively building knowledge)
      4.  **Conclusion & Next Steps:** Summarize and suggest how to continue learning or apply knowledge.
      
      Use clear, bolded headings for modules and sections. Ensure practice tasks are conceptual/design-focused, as actual lab environments are not simulated here. Keep the quizzes simple.`;

      const response = await fetch('https://backend-project-190.onrender.com/api/ai/generate-cloud-course', {
        method: 'POST',
      ...
      setMessages(prevMessages => [...prevMessages, { sender: 'ai', text: data.courseContent || "Sorry, I couldn't generate a course for that goal." }]);
  ...
  return (
    ...
    >
      <h1 className={`text-3xl sm:text-4xl font-bold mb-8 ${headerColorClass}`}>
        CloudCoach: AI-Powered Cloud Course Generator
      </h1>
      ...
      <div className={`flex-grow flex flex-col ${chatContainerBgClass} rounded-lg shadow-lg border border-gray-700 overflow-hidden`}>
        <div className="p-4 border-b border-gray-700 text-lg font-semibold text-center text-blue-300">
          Tell me what cloud topic or skill you want to learn, and I'll generate a personalized course outline!
        </div>
        <div className="flex-grow p-4 overflow-y-auto space-y-4">
          {messages.length === 0 && (
            ...
            >
              Hello! I'm your AI Cloud Course Generator. What would you like to learn today?
              <p className="text-xs mt-1 opacity-80">
                (e.g., "Learn how to build a serverless REST API on AWS", "Understand VPC networking basics", "Master EC2 instance management and scaling")
              </p>
            </motion.div>
          )}
        ...
        <form onSubmit={handleGenerateCourse} className="p-4 border-t border-gray-700 flex">
          <textarea
            value={userGoal}
            onChange={(e) => setUserGoal(e.target.value)}
            placeholder="e.g., 'How to build a simple serverless API', 'Fundamentals of AWS networking', 'Deep dive into S3 bucket policies'"
...
export default CloudCourseGenerator;

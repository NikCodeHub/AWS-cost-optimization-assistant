// src/pages/InteractiveCloudLabs.jsx
import React, { useState, useRef, useEffect } from 'react';
import { motion } from 'framer-motion';

const InteractiveCloudLabs = ({ isDarkMode }) => {
  const [messages, setMessages] = useState([]);
  const [userLabTask, setUserLabTask] = useState('');
  ...
  const handleGenerateLab = async (e) => {
    e.preventDefault();
    if (!userLabTask.trim()) return;

    const newUserMessage = { sender: 'user', text: userLabTask.trim() };
    setMessages(prevMessages => [...prevMessages, newUserMessage]);
    setUserLabTask('');
    setLoading(true);

    try {
      const promptContext = `As an expert AI Cloud Lab Simulator and Virtual Instructor, your task is to guide the user through a simulated interactive lab experience based on their described task.
      ...
      **Instructions:**
      ...
      2.  **Step 2: [Action Description]**
          ... (repeat for 3-5 logical steps)
      **Verification:** (How can the user conceptually verify their work?)
      **AI Feedback/Tips:** (Optional, provide a tip or common pitfall for this lab)
      
      Ensure the commands are realistic for AWS CLI, even if simulated. Use placeholder values for sensitive information (e.g., bucket-name-unique).`;

      const response = await fetch('https://backend-project-190.onrender.com/api/ai/interactive-cloud-lab', {
        method: 'POST',
      ...
      setMessages(prevMessages => [...prevMessages, { sender: 'ai', text: data.labContent || "Sorry, I couldn't generate a simulated lab for that task." }]);

    } catch (error) {
      console.error("Error generating AI lab:", error);
  ...
  return (
    ...
    >
      <h1 className={`text-3xl sm:text-4xl font-bold mb-8 ${headerColorClass}`}>
        CloudCoach: Interactive Cloud Labs (Simulated)
      </h1>
      ...
      <div className={`flex-grow flex flex-col ${chatContainerBgClass} rounded-lg shadow-lg border border-gray-700 overflow-hidden`}>
        <div className="p-4 border-b border-gray-700 text-lg font-semibold text-center text-blue-300">
          Describe an AWS task you want to practice, and I'll simulate a lab for you!
        </div>
        <div className="flex-grow p-4 overflow-y-auto space-y-4">
          {messages.length === 0 && (
            ...
            >
              Hello! I'm your Virtual Cloud Lab Instructor. What AWS task would you like to practice today?
              <p className="text-xs mt-1 opacity-80">
                (e.g., "How do I create a VPC with public and private subnets?", "Simulate creating an S3 bucket with versioning", "Guide me through attaching an IAM role to an EC2 instance.")
              </p>
            </motion.div>
          )}
          ...
          {loading && (
            ...
            >
              <div className="flex items-center">
                ...
                Simulating lab steps...
              </div>
            </motion.div>
          )}
          <div ref={messagesEndRef} />
        </div>
        <form onSubmit={handleGenerateLab} className="p-4 border-t border-gray-700 flex">
          <textarea
            value={userLabTask}
            onChange={(e) => setUserLabTask(e.target.value)}
            placeholder="e.g., 'Create an S3 bucket with public read access', 'Setup an EC2 instance with a security group allowing SSH', 'Configure a CloudWatch alarm for CPU utilization'"
...
export default InteractiveCloudLabs;
